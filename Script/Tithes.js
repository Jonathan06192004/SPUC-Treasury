...
function fmtVariance(v25, v26) {
  ...
  const diff = v26 - v25, pct = ((diff/v25)*100).toFixed(1);
  const cls = diff >= 0 ? 'variance-pos' : 'variance-neg';
  const arrow = diff >= 0 ? '▲' : '▼';
  return `<td class="cell-variance ${cls}">${arrow} ${fmt(Math.abs(diff))}<br><span class="variance-pct">${arrow} ${Math.abs(pct)}%</span></td>`;
}
...
function renderPage(data2025, data2026, budgetData, missionData, missionData2026) {
  ...
  document.getElementById('excelBody').innerHTML =
    ...
    `<tr class="total-row"><td class="month-cell">Total</td><td class="total-cell">${fmt(t25)}</td><td class="total-cell">${fmt(t26)}</td><td class="total-cell">${fmt(tBgt)}</td><td class="total-cell ${tCls}">${tArrow} ${fmt(Math.abs(tDiff))}<br><span class="variance-pct">${tArrow} ${Math.abs(tPct)}%</span></td></tr>`;

  const canvas = document.getElementById('tithesChart');
